...
in WebGL and orbitable (auto-spin + drag + zoom) — plus four 2D-canvas cores:
a **nixie tube**, a **seven-segment digit**, an **electromechanical flip-dot
board** (discs physically rotate, and click), and a **split-flap (Solari)
display** (flap cards fall from a drum, and clack). All ship as
...
a solenoid rattle on `/flipdot`, and a split-flap departures board — with chroma
image drums, matrix rain, snake and pong — on `/splitflap`.
...
| [`@glowbox/svelte`](packages/svelte)               | `yarn add @glowbox/svelte`        | Svelte 5 components, one per core (`<LedGrid>` … `<SplitFlap>`)        |
| [`@glowbox/react`](packages/react)                 | `yarn add @glowbox/react`         | React components: the same five (`^18 \|\| ^19`)                       |
| [`@glowbox/vue`](packages/vue)                     | `yarn add @glowbox/vue`           | Vue 3 components: the same five                                        |
| [`@glowbox/extras`](packages/extras)               | `yarn add @glowbox/extras`        | content: GIF/image players, 5×7 LED font text + scroller               |

Five rendering cores — the 3D LED grid (`@glowbox/led-grid`), the nixie tube
(`@glowbox/nixie`), the seven-segment display (`@glowbox/seven-segment`), the
flip-dot board (`@glowbox/flip-dot`) and the split-flap display
(`@glowbox/split-flap`) — and each framework package (`svelte`/`react`/`vue`)
ships a thin component for **all of them** (`<LedGrid>` + `<NixieTube>` +
`<SevenSegment>` + `<FlipDots>` + `<SplitFlap>`).
...
examples/svelte-gallery  the demo SPA (LED gallery + nixie/seven/flipdot/splitflap) → GitHub Pages
```

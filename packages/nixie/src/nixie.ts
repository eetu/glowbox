...
export interface NixieOptions {
	 ...
	 *  The glow is drawn source-over on a cleared (alpha-0) canvas, so it reads as straight
	 *  (un-premultiplied) alpha on any background. `mesh` / `ghost` stay independent. */
	bare?: boolean;
}
...
export function glyphPath(symbol: string | number | null): string | null {
	const ch = norm(symbol);
	if (ch === ':') return colonD || null;
	return GLYPHS[ch] || null;
}

/** The coordinate space of `glyphPath` data (px, y-down). */
export const GLYPH_VIEWBOX = { width: VB_W, height: VB_H } as const;

/** One cathode in a tube's physical stack: its glyph centreline, its 0-based depth
 *  (front → back), and the small in-plane drift the tube gives it. */
export interface NixieCathode {
	symbol: string;
	path: string;
	/** 0 (front) .. 9 (back) — map to a z offset when building a 3D stack. */
	depth: number;
	/** Sideways drift in glyph-viewBox units (the parallax the 2D tube uses). */
	offset: [number, number];
}

/** The full digit cathode stack (0–9) in physical front→back order. Build a 3D tube by
 *  extruding every cathode's `path` at its `depth`, then lighting the current value's —
 *  so from any angle the whole stack is present and one numeral glows among the rest. */
export function nixieCathodes(): NixieCathode[] {
	return [...STACK].map((ch) => {
		const { ox, oy } = placement(ch);
		return { symbol: ch, path: GLYPHS[ch] ?? '', depth: DEPTH[ch], offset: [ox, oy] };
	});
}

/** A tube style's proportions: `[squashX, squashY]` and the filament `strokeWidth` (in
 *  glyph-viewBox units) — use it as the 3D wire's diameter so thickness matches the 2D tube. */
export function nixieStyle(style: NixieStyle): { squash: [number, number]; strokeWidth: number } {
	const s = STYLES[style];
	return { squash: [s.sx, s.sy], strokeWidth: s.lw };
}

/** A honeycomb anode-mesh cell centre. */
export interface NixieMeshCell {
	x: number;
	y: number;
}

/** The honeycomb anode-mesh layout over a `width`×`height` tube face (same coordinate
 *  convention as the glyph viewBox), for building the grille in 2D or 3D. `radius` is the
 *  hex circumradius; centres are in the 0..width / 0..height space (y-down). */
export function nixieMesh(
	width: number,
	height: number
): { radius: number; cells: NixieMeshCell[] } {
	const radius = Math.max(width, height) * 0.06;
	const hw = radius * Math.sqrt(3);
	const cells: NixieMeshCell[] = [];
	for (let y = -radius; y < height + radius; y += radius * 1.5) {
		const off = (Math.round(y / (radius * 1.5)) % 2) * (hw / 2);
		for (let x = -hw; x < width + hw; x += hw) cells.push({ x: x + off, y });
	}
	return { radius, cells };
}

/** The dull-nickel colour of the unlit cathode wires (`[r,g,b]` 0..1). */
export const NIXIE_WIRE_COLOR: readonly [number, number, number] = [WIRE[0], WIRE[1], WIRE[2]];
...
export function createNixieTube(
	...
	function drawMesh(bw: number, bh: number) {
		...
		ctx!.lineWidth = 0.8;
		const { radius, cells } = nixieMesh(bw, bh);
	...
	function draw() {
		...
		// Clip to the glass and work in glass-local coordinates (bare: whole canvas, no clip).
		g.save();
		if (!bare) {
			roundRect(padX, padY, bw, bh, rad);
			g.clip();
		...
		// Glass highlight (skip on tiny tubes, and in bare mode — it belongs to the glass).
		if (!micro && !bare) {
		...
		// against a light background. Skipped in bare mode (no glass module).
		if (!bare) {
			roundRect(padX, padY, bw, bh, rad);
			g.lineWidth = 1;

...
# peer: svelte ^5   (the eight display cores come along as dependencies)
```
...
```svelte
<script lang="ts">
	import { LcdModule } from '@glowbox/svelte';
</script>
...
`pixelRatio`, `label` mirror the core options and update **live**; `oncreate` hands you
the imperative `LcdModule` handle (`setGlyph` for CGRAM custom characters, `setCursor`,
`power`, `cellAt`/`cellRect` hit-testing). See **@glowbox/lcd** for the panels and the
crystal physics.

...
// `a1 1 0 011 0`); curves flatten adaptively with a tolerance scaled to the path's own
// bounding box, so a 24-unit icon and a 512-unit drawing both come out smooth.
//
// What each core does with the result is NOT shared, because it is the one real difference
// between them: neon needs open CENTRELINES (a tube is a stroke) and vfd needs closed
// RINGS (an anode is a printed patch, so a fill). Each owns that tail in its own
// `path.ts`; this file stops at the geometry.
...
/** One flattened subpath in the path's own units. `closed` is whether the data carried a
 *  `Z` — what that should mean is the caller's business. */
export interface FlatSub {
	pts: Pt[];
	closed: boolean;
}

/** Parse and flatten path data into subpaths. Throws on malformed data. */
export function flattenPath(d: string | string[], opts: PathFlattenOptions = {}): FlatSub[] {

...
// Shared by: crt (type only), flip-dot, lcd, led-grid, neon, nixie, seven-segment, split-flap, vfd.
//
// A `Color` is either a numeric RGB triple in 0..1 (values >1 are allowed, and bloom
// under led-grid's additive glow) or any CSS colour string ('#ff8800', 'rgb(0 128 255)',
// 'tomato', 'hsl(...)', …). Everything is normalised to a plain `RGB` before it reaches a
// buffer or a fill.
//
// Note: only the array form can exceed 1 (bloom); CSS strings are 0..255 → 0..1.
//
// `Vec3` and `parseColor01` are led-grid's — the 2D cores don't import them and they
// tree-shake away, which their size budgets guard.

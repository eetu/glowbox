// Demo driver: a self-playing 3D Pac-Man on the @glowbox/led-grid LED-grid display,
// now with a real maze so the ghost personalities actually matter (in an
...
import type { LedDisplay, Vec3 } from '@glowbox/led-grid';
...
// A firework particle (float position/velocity in grid space) for the win celebration.
type Spark = {
	x: number;
	y: number;
	z: number;
...
export function makePacman(n = 9) {
	...
	const SCATTER = 5.0;
	const CHASE = 16.0;
	const WIN = 4.0; // seconds of fireworks when the maze is cleared
	const total = n * n * n;
	...
	let winT = 0; // win-celebration (fireworks) timer
	let fwTimer = 0; // time until the next firework burst
	let sparks: Spark[] = [];
	...
	function gameStep() {
		...
		if (pillsLeft <= 0) {
			winT = WIN; // maze cleared → celebrate
			sparks = [];
			fwTimer = 0;
		}
	}

	// --- win celebration: 3D fireworks on a cleared grid ---
	function burst() {
		const cx = 1 + Math.random() * Math.max(1, n - 2);
		const cy = n * 0.55 + Math.random() * (n * 0.35); // upper half
		const cz = 1 + Math.random() * Math.max(1, n - 2);
		const [r, g, b] = FW_COLORS[(Math.random() * FW_COLORS.length) | 0];
		const count = Math.min(70, Math.max(20, Math.round(n * 2.2)));
		const speed = Math.max(2, n * 0.5);
		for (let i = 0; i < count; i++) {
			// random direction on the unit sphere, varied speed
			const u = Math.random() * 2 - 1;
			const th = Math.random() * Math.PI * 2;
			const rxy = Math.sqrt(1 - u * u);
			const sp = speed * (0.45 + Math.random() * 0.7);
			const max = 0.9 + Math.random() * 0.7;
			sparks.push({
				x: cx,
				y: cy,
				z: cz,
				vx: rxy * Math.cos(th) * sp,
				vy: u * sp,
				vz: rxy * Math.sin(th) * sp,
				r,
				g,
				b,
				life: max,
	...
	function fireworks(d: LedDisplay, dt: number) {
		fwTimer -= dt;
		if (fwTimer <= 0 && winT > 0.7) {
			burst();
			fwTimer = 0.3 + Math.random() * 0.4;
		}
		const grav = n * 0.55;
		d.clear();
		for (let i = sparks.length - 1; i >= 0; i--) {
			...
			const k = (s.life / s.max) * 1.5; // fade out; >1 blooms in hologram
			d.add(x, y, z, [s.r * k, s.g * k, s.b * k]);
		}
	}

	return (d: LedDisplay, dt: number) => {
		clock += dt;
		// Win celebration: play fireworks, then start a fresh level (keeping lives).
		if (winT > 0) {
			winT -= dt;
			fireworks(d, dt);
			if (winT <= 0) {
		...
		acc += dt;
		let steps = 0;
		while (acc >= STEP && steps < 4 && winT <= 0) {
			acc -= STEP;
			gameStep();
			steps++;
		}

		// entity afterglow (short, subtle tail → animates the discrete steps without
		// smearing; shorter time-constant = a tighter trail)
		...
		deposit(pac.p, [1.15, 1.05, 0.1]); // yellow-white — distinct from orange Clyde

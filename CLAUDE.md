...
```text
packages/
  core/     @glowbox/core   — plain-TS WebGL display + canvas-like voxel API. ZERO deps.
  svelte/   @glowbox/svelte — Svelte 5 <LedGrid> wrapper (peer: svelte; dep: @glowbox/core).
examples/
...
- **The library is display-only.** `@glowbox/core` = a framework-agnostic core
...
- **Packaging.** `@glowbox/core` builds with Vite lib mode + `vite-plugin-dts`;
...
- `yarn build` — topological: `@glowbox/core` → `@glowbox/svelte` → demo.
- `yarn test` / `yarn validate` (= lint + format + typecheck + test). Yarn is
  vendored: `node .yarn/releases/yarn-*.cjs <script>`.
- Per-package: `yarn workspace @glowbox/core <script>`, etc.
...
- **Restructured from the old sibling-app** (Rust backend + SvelteKit SPA) into
  ...
  into `@glowbox/core` + `@glowbox/svelte`; demo moved to `examples/`.

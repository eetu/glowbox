...
packages/
  ...
  flip-dot/  @glowbox/flip-dot — 2D-canvas electromechanical board: physical disc/vane flips, scan-wave stagger, ditherFrame, shared-AudioContext solenoid click (createMechSound = split-flap groundwork), `shaded` depth priced by THEME (split-flap's SHADE convention: soften + warm-grey ink; per-theme bits = lit facets need MORE white on pale plastic, stop-post metal, edge-on sliver lit-vs-shade). Zero deps.
  split-flap/ @glowbox/split-flap — 2D-canvas Solari board: drum-of-cards model (forward-only wraps), perspective falls, grapheme drums + drum zones (per-field drums) + chroma drums (chromaDrum/paletteFrame/FlapFace), card-slap sound (vendored createMechSound + noise shaping), `shaded` fallen pile = one whole CARD per rib (own rounded silhouette at the flap's radius, deterministic off-register scatter — sin-hash, never Math.random, or a resize reshuffles the pile — face climbs out of the crevice into a lit bottom edge that HUGS the silhouette so it rounds at the corners instead of ruling edge-to-edge; nothing paints above the flap's bottom edge or the resting flap loses its own corners), `shaded` flight physics = the front face brightens swinging up toward the light (the strip added to ITSELF at low alpha — masks to the silhouette, scales with albedo) while the back face arrives floor-facing dark and recovers landing, the cast shadow sweeps flap + pile, and the card gradient BREAKS at the seam (two flaps at two tilts, not one printed strip), `shaded` depth priced by THEME (one `soften` factor + a warm-grey shadow ink: a shadow belongs to the room, so the pale board runs every shadow the code asks for at ~47% density — dark-theme black over cream read as ink-lined tiles and a metallic wash down each flap; the three unscalable bits are the fallen ribs' lit edge, which needs MORE white on a pale card, the hinge-clip metal, and the edge-on card thickness, lit on a dark board and shaded on a pale one). Zero deps.
  neon/      @glowbox/neon     — 2D-canvas glass-tube neon sign: single-stroke tube letterforms (vendored Hershey script+sans faces, custom NeonFont escape hatch), sign art (SVG path data → tubes via pathToStrokes, placed behind/beside the text, own gas/colour + flasher circuit, shared `frame` registers multi-piece drawings, `opaque` faces cut rear tubes for real overlap), visible unlit glass, electrode strike sequences, wear arc, rate-capped flasher programs, painted letter faces (`face`, per line or per art piece — the skeleton stroked SLAB_R wide, washed by ITS OWN circuit's level brightest beside the glass, never as bright as it; absorb catches nothing), outlined letters (`outline`, per line — the tube bent AROUND the slab: offset contours cut against sibling-stroke slabs via the coverage trick, counters included, square-cut terminals, skeleton kept on the section for the paint; pair with letterSpacing, text-only in practice), the word channel (words are circuits: `TubeSection.word` counts across the text; `wordOn` switches one, `wordColor` paints one, `lineScale` sizes a line's typography while tube width holds — the HOTEL / red-NO / blue-VACANCY motel sign; a chase/dying-letter demo still wants `tubes: 'glyph'`), `polarity: 'absorb'` (the one invented element — tubes that ink a pale wall, per sign or per art piece, so neon works in a light theme; ink composites on one layer per frame, never per pass; each gas carries its OWN ink in the table — that's where co2's white inverts to a black light — while a colour you NAME is the pigment (`pigment()`), so lightness means the same thing inking as lit and a pale track under a bright meter stays two tubes; the hot core is a second coat (gasCol squared), never neutral black, and ink density falls with the pass's blur so a pale wall doesn't fog between tubes), circuit wiring (a section is one bent tube: electrode pair ALWAYS routed to the run's true free ends via `circuitEnds` — never font-data order; interior stroke ends bare glass diving behind; `tubes` is the ONE granularity knob — 'auto' wires per WORD, 'glyph' restores channel-letter tubes/strikes/wear; the returns are never drawn — auto-routing them as visible geometry was tried and rejected, it reads as mess), tappable tubes (`sectionAt` + `jolt`, split-flap's geometry-only contract), transformer hum (createHum in the vendored sound engine; Hershey ack ships as HERSHEY_LICENSE + LICENSE-hershey). Zero deps.
